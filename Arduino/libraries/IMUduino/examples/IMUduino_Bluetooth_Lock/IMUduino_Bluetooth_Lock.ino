...
// 
// Modified slightly to work with Leo compatible IMUduino board - Femtoduino.com 2014-09-05
// This revision does not use LEDs. Just an IMUDuino, and a Solenoid lock.
//
#include <SPI.h>
...
#define ADAFRUITBLE_RDY 7 // interrupt pin 2 or 3 on UNO, pin D7 on IMUduino
...
void setup() {
  //while(!Serial); // Wait for serial object, (Leonardo board)
  Serial.begin(9600);
...
void loop() {
  ...
  if (status == ACI_EVT_CONNECTED) {
    ...
    if (BTLEserial.find("u")) {
      ...
      Serial.println("Found input with u");
